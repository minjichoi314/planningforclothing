...
export function createServer(){return http.createServer(async(req,res)=>{
  if(req.method==='POST'&&req.url==='/api/feedback'){
    ...
    try{
      ...
      let response;try{response=await fetch('https://api.openai.com/v1/responses',{method:'POST',headers:{'Authorization':`Bearer ${process.env.OPENAI_API_KEY}`,'Content-Type':'application/json'},body:JSON.stringify({model:process.env.OPENAI_MODEL||'gpt-4.1-mini',store:false,max_output_tokens:650,instructions:`당신은 한국 중학생의 의복 마련 계획을 돕는 ${data.career}입니다. 점수 판정이나 구매 지시를 하지 마세요. 학생의 실제 조사 답변을 근거로 장점 1개, 더 조사할 점 2개, 계획 수정 제안 2개를 따뜻하고 구체적인 한국어로 작성하세요. 가격·소재·인증 사실을 검증하지 못했다면 확인 필요하다고 명시하세요. 개인정보를 반복하지 마세요. 700자 이내의 일반 텍스트로 답하세요.`,input:JSON.stringify({직업:data.career,미션:data.steps.map((s,i)=>({단계:i+1,답변:s.answers}))})}),signal:controller.signal});}finally{clearTimeout(timer);}
